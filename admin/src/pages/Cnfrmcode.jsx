...
const Cnfrmcode = () => {
    ...
    const hndlclk = async (e) => {
        e.preventDefault()
        const {admunam, admcnfcode} = vl

        try {
            const res = await axios.post(`${server}/admin/cnfcode`, {admunam, admcnfcode})
            const dta = await res.data
            // console.log(dta)

            if (dta.statuscode === 220) {
                window.alert(dta.success)
                navig(`/admin/resetpass/${dta.data._id}`)
            }
            else {
    ...
    <>
        <div className='lgpg'>
            <div className="login-box">
                <span className='hdr'>Confirm OTP Code</span>
                <form>
                    ...
                    <div className="user-box">
                        <input type="text" id='admcnfcode' name="admcnfcode" required={true} onChange={hndlinp} />
                        <label htmlFor='admcnfcode'>Confirmation Code</label>
                    </div>
                    <div className="user-box" style={{display:'flex',justifyContent:'space-between',flexDirection:'row',width:'100%'}}>
                        <div className="frgt" style={{backgroundColor:'darkred',padding:'0.5rem 1rem',color:'lightcoral',fontSize:'1rem',border:'none',borderRadius:'0.2rem'}}>
                            <NavLink to='/admin/forgetpass'>Back</NavLink>
                        </div>
